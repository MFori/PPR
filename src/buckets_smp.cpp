...
/*void run_pipeline(int ntoken, std::ifstream *file) {
    tbb::parallel_pipeline(tbb::flow::unlimited,
                           tbb::make_filter<void, double>(
                                   tbb::filter_mode::serial_in_order, SMPFileReader(file)
                           )
                           &
                           tbb::make_filter<double, double>(
                                   tbb::filter_mode::serial_in_order, process()
                           )
                           &
                           tbb::make_filter<double, void>(
                                   tbb::filter_mode::serial_in_order, create_buckets_smp(file, nullptr, nullptr)
                           )
    );
}*/
